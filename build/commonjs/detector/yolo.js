...
var modelLoader_1 = require("../utils/modelLoader");
var preProcess_1 = require("../utils/preProcess");
var YOLODetector = /** @class */ (function () {
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _b.trys.push([0, 2, , 3]);
                        _a = this;
                        return [4 /*yield*/, modelLoader_1.loadModel(this.modelURL)];
                    case 1:
                        _a.model = _b.sent();
                        return [3 /*break*/, 3];
                    case 2:
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        dummy = tf_1.tf.zeros([this.modelSize, this.modelSize, 3]);
                        return [4 /*yield*/, this.detect(dummy)];
                    case 1:
                        _a.sent();
                        tf_1.tf.dispose(dummy);
                        return [3 /*break*/, 3];
                    case 2:
                        error_2 = _a.sent();
                        throw error_2;
    ...
    YOLODetector.prototype.detectSyncCPU = function (image) {
        ...
        tf_1.tf.dispose(results);
        var prev = tf_1.tf.getBackend();
        tf_1.tf.setBackend('cpu');
        var _a = __read(this.getMaxScoresAndClasses(scoreArray, numBoxes, numClasses), 2), maxScores = _a[0], classes = _a[1];
        var boxes2 = tf_1.tf.tensor2d(boxArray, [results[0].shape[0], results[0].shape[1]]);
        var indexTensor = tf_1.tf.image.nonMaxSuppression(boxes2, maxScores, this.maxOutput, this.iouThreshold, this.classProbThreshold);
        var indexArray = indexTensor.dataSync();
        tf_1.tf.dispose(boxes2);
        tf_1.tf.dispose(indexTensor);
        var finalBoxes = this.createFinalBoxes(boxArray, maxScores, classes, indexArray);
        tf_1.tf.setBackend(prev);
        return finalBoxes;
    };
    YOLODetector.prototype.detectCPU = function (image) {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0: return [4 /*yield*/, tf_1.tf.nextFrame()];
                    case 1:
                        _b.sent();
                    ...
                    case 3:
                        scoreArray = _b.sent();
                        tf_1.tf.dispose(results);
                        prev = tf_1.tf.getBackend();
                        tf_1.tf.setBackend('cpu');
                        _a = __read(this.getMaxScoresAndClasses(scoreArray, numBoxes, numClasses), 2), maxScores = _a[0], classes = _a[1];
                        boxes2 = tf_1.tf.tensor2d(boxArray, [results[0].shape[0], results[0].shape[1]]);
                        return [4 /*yield*/, tf_1.tf.image.nonMaxSuppressionAsync(boxes2, maxScores, this.maxOutput, this.iouThreshold, this.classProbThreshold)];
                    case 4:
                        indexTensor = _b.sent();
    ...
    YOLODetector.prototype.detectSync = function (image) {
        var _this = this;
        var _a = __read(tf_1.tf.tidy('', function () {
        ...
        tf_1.tf.dispose([boxes, scores, classes]);
    ...
    YOLODetector.prototype.draw = function (detections, canvas) {
        ...
        draw_1.draw(detections, ctx, lablesLen);
    };
    /**
    ...
    YOLODetector.prototype.predictInternal = function (image) {
        var _this = this;
        return tf_1.tf.tidy(function () {
            var _a = __read(preProcess_1.preProcess(image, _this.modelSize, _this.resizeOption), 3), imgWidth = _a[0], imgHeight = _a[1], data = _a[2];
    ...
    YOLODetector.prototype.postProcessRawPrediction = function (rawPrediction) {
        ...
        for (var i = 0; i < layers.length; i++) {
            ...
            var anchorsTensor = tf_1.tf.tensor(anchorsArr).reshape([1, 1, numAnchors, 2]);
        ...
        var boxesTensor = tf_1.tf.concat(boxes, 0);
        var probsTensor = tf_1.tf.concat(probs, 0);
    ...
    YOLODetector.prototype.processLayer = function (prediction, anchorsTensor, modelSize, classesLen, version) {
        ...
        var reshaped = tf_1.tf.reshape(prediction, [outputWidth, outputHeight, anchorsLen, classesLen + 5]);
        var boxxy = tf_1.tf.sigmoid(reshaped.slice([0, 0, 0, 0], [outputWidth, outputHeight, anchorsLen, 2]));
        var boxwh = tf_1.tf.exp(reshaped.slice([0, 0, 0, 2], [outputWidth, outputHeight, anchorsLen, 2]));
        var boxConfidence = tf_1.tf.sigmoid(reshaped.slice([0, 0, 0, 4], [outputWidth, outputHeight, anchorsLen, 1])).reshape([numBoxes, 1]);
        var boxClassProbs = tf_1.tf.softmax(reshaped.slice([0, 0, 0, 5], [outputWidth, outputHeight, anchorsLen, classesLen]))
            .reshape([numBoxes, classesLen]);
        var classProbs = tf_1.tf.mul(boxConfidence, boxClassProbs);
        // prep
        var boxIndex = tf_1.tf.range(0, outputWidth);
        var boxHeightIndex = tf_1.tf.tile(boxIndex, [outputHeight]);
        var boxWidthindex = tf_1.tf.tile(tf_1.tf.expandDims(boxIndex, 0), [outputWidth, 1]).transpose().flatten();
        var boxIndexGrid = tf_1.tf.transpose(tf_1.tf.stack([boxHeightIndex, boxWidthindex])).reshape([outputWidth, outputHeight, 1, 2]);
        var convDims = tf_1.tf.reshape(tf_1.tf.tensor1d([outputWidth, outputHeight]), [1, 1, 1, 2]);
        // end
        boxxy = tf_1.tf.div(tf_1.tf.add(boxxy, boxIndexGrid), convDims);
        boxwh = tf_1.tf.mul(boxwh, anchorsTensor);
        if (version === 'v3') {
            boxwh = tf_1.tf.div(boxwh, tf_1.tf.tensor([modelSize]));
        }
        else {
            boxwh = tf_1.tf.div(boxwh, convDims);
        }
    ...
    YOLODetector.prototype.boxesToCorners = function (boxXY, boxWH) {
        var two = tf_1.tf.scalar(2);
        var boxMins = tf_1.tf.sub(boxXY, tf_1.tf.div(boxWH, two));
        var boxMaxes = tf_1.tf.add(boxXY, tf_1.tf.div(boxWH, two));
        var dim0 = boxMins.shape[0];
        var dim1 = boxMins.shape[1];
        var dim2 = boxMins.shape[2];
        var size = [dim0, dim1, dim2, 1];
        return tf_1.tf.concat([
    ...
    YOLODetector.prototype.rescaleBoxes = function (boxes, imgWidth, imgHeight) {
        var width = tf_1.tf.scalar(imgWidth);
        var height = tf_1.tf.scalar(imgHeight);
        // this for y1 x1 y2 x2
        var imageDims = tf_1.tf.stack([height, width, height, width]).reshape([1, 4]);
        // this for x y w h
    ...
    YOLODetector.prototype.filterBoxes = function (boxes, scores, classProbThresh) {
        var boxScore = tf_1.tf.max(scores, -1);
        var boxClasses = tf_1.tf.argMax(scores, -1);
        // score filter mask
        var filterThresh = tf_1.tf.scalar(classProbThresh);
        var filterMask = tf_1.tf.greaterEqual(boxScore, filterThresh);
        // this is somewhat a replacment for tf.boolean_mask
        var indicesTensor = tf_1.tf.linspace(0, boxes.shape[0] - 1, boxes.shape[0]).toInt();
        var negativeIndicesTensor = tf_1.tf.fill([boxes.shape[0]], -1, 'int32');
        var indices = tf_1.tf.where(filterMask, indicesTensor, negativeIndicesTensor);
        var filteredIndicesTensor = tf_1.tf.tensor1d(indices.dataSync().filter(function (i) { return i >= 0; }));
    ...
    YOLODetector.prototype.filterBoxesAsync = function (boxes, scores, classProbThresh) {
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        boxScore = tf_1.tf.max(scores, -1);
                        boxClasses = tf_1.tf.argMax(scores, -1);
                        filterThresh = tf_1.tf.scalar(classProbThresh);
                        filterMask = tf_1.tf.greaterEqual(boxScore, filterThresh);
                        range = tf_1.tf.linspace(0, boxes.shape[0] - 1, boxes.shape[0]);
                        indicesTensor = range.toInt();
                        negativeIndicesTensor = tf_1.tf.fill([boxes.shape[0]], -1, 'int32');
                        indices = tf_1.tf.where(filterMask, indicesTensor, negativeIndicesTensor);
                        _b = (_a = tf_1.tf).tensor1d;
                    ...
                    case 1:
                        ...
                        tf_1.tf.dispose([boxScore, boxClasses, filterThresh, filterMask,
